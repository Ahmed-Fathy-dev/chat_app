  ...
  factory Failure.fromCode(dynamic error) {
    ...
    switch (error.type) {
      case DioErrorType.connectTimeout:
        ResponseMessage.CONNECT_TIMEOUT.logE('Network Exeption');
        return const Failure(
          ResponseCode.CONNECT_TIMEOUT,
          ResponseMessage.CONNECT_TIMEOUT,
        );
      case DioErrorType.sendTimeout:
        ResponseMessage.SEND_TIMEOUT.logE('Network Exeption');
        return const Failure(
          ResponseCode.SEND_TIMEOUT,
          ResponseMessage.SEND_TIMEOUT,
        );
      case DioErrorType.receiveTimeout:
        ResponseMessage.RECEIVE_TIMEOUT.logE('Network Exeption');
        return const Failure(
      ...
      case DioErrorType.response:
        if (error.response != null &&
          ...
          error.response?.statusMessage.toString().logE('Network Exeption');
          return Failure(
      ...
      case DioErrorType.cancel:
        ResponseMessage.CANCEL.logE('Network Exeption');
        return const Failure(ResponseCode.CANCEL, ResponseMessage.CANCEL);
      case DioErrorType.other:
        if (error.error.toString().contains('SocketException')) {
          ResponseMessage.NO_INTERNET_CONNECTION.logE('Network Exeption');
